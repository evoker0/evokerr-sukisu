...
COMMON_REFS   = "https://android.googlesource.com/kernel/common/+refs/heads/?format=JSON"
...
def months_per_kmi(token=None):
    """Months that can still actually be built.

    The manifest repo keeps every dated branch forever - common-android12-5.10-2023-03 is
    still listed - but each one pins kernel/common at a branch of the same name, and
    kernel/common only keeps roughly the last five to seven. So most of those manifest
    branches now point at something that no longer exists and cannot be synced at all.
    Other builders' old assets are historical builds from when those branches were alive,
    not something reproducible today.

    Intersecting the two lists is what makes the difference between a matrix of 195 mostly
    impossible builds and one of ~35 real ones.
    """
    manifest = gitiles_json(MANIFEST_REFS, token)
    common = gitiles_json(COMMON_REFS, token)
    out = {}
    for kmi, _, _ in KMIS:
        mpat = re.compile(r"^common-" + re.escape(kmi) + r"-(\d{4}-\d{2})$")
        listed = set(m.group(1) for m in (mpat.match(n) for n in manifest) if m)
        cpat = re.compile(r"^" + re.escape(kmi) + r"-(\d{4}-\d{2})$")
        alive = set(m.group(1) for m in (cpat.match(n) for n in common) if m)
        gone = len(listed - alive)
        if gone:
            print("  %-16s %d month(s) in the manifest no longer exist in kernel/common"
                  % (kmi, gone), file=sys.stderr)
        out[kmi] = sorted(listed & alive, reverse=True)
    return out
...
def main():
    ...
    for kmi, av, kv in KMIS:
        ...
        # Group months by sublevel. Several consecutive monthly branches usually carry the
        # same kernel, so building each of them separately means building the same kernel
        # repeatedly - and because the build falls back to an older month when susfs does
        # not fit, two of those jobs can resolve to the same month and overwrite each
        # other's asset. One entry per sublevel, with its months as a fallback list, keeps
        # every filename unique and does the work once.
        by_sub = {}
        for month in chosen:
            by_sub.setdefault(sublevel(kmi, month, subs, token), []).append(month)
        by_sub.pop("0", None)      # sublevel unknown, skip rather than name a file *.0

        print("%-16s %d month(s) -> %d sublevel(s)" % (kmi, len(chosen), len(by_sub)),
              file=sys.stderr)

        for sub, subs_months in by_sub.items():
            subs_months.sort(reverse=True)
            newest = subs_months[0]
            for root in wanted:
                key = "%s|%s|%s" % (kmi, sub, root)
                stamp = "%s|%s" % (newest, revs[root])
                ...
                matrix.append({
                    ...
                    "sub_level": sub, "os_patch_level": newest,
                    "months": ",".join(subs_months[:6]),
    ...
    keep = set("%s|%s|%s" % (e["kmi"], e["sub_level"], e["root_solution"]) for e in matrix)
    merged = dict(built)
